...
/*
    ...
    命令godoc是一个很强大的工具，用于展示指定代码包的文档。我们可以通过运行go get code.google.com/p/go.tools/cmd/godoc安装它。

	该命令有两种模式可供选择。如果在执行命令时不加入-http标记，则该命令就以命令行模式运行。在打印纯文本格式的文档到标准输出后，命令执行就结束了。比如，我们用命令行模式查看代码包fmt的文档：

	hc@ubt:~$ godoc fmt
	由于篇幅原因，我们在本小节中略去了文档查询结果。读者可以自己运行一下上述命令。在该命令被执行之后，我们就可以看到编排整齐有序的文档内容了。这包括代码包fmt的综述和所有可导出成员的声明、文档以及例子。
	...
	如果我们不但想在文档中查看可导出成员的声明，还想看到它们的源码，那么我们可以在执行godoc命令的时候加入标记-src，比如这样：
	...
	注意，要使用-index标记开启搜索索引，这个索引会在服务器启动时创建并维护。否则无论在Web页面还是命令行终端中提交查询都会返回错误“Search index disabled: no results available”。
...
//	@Param   signedness
//	@Param   digits
func Socket2(a int64, base uint64, signedness bool, digits string) {
